...
METRICS_FILE = "../../csv/har_metrics.csv"

KEY_COLUMNS = ["country", "website"]

METRIC_COLUMNS = [
    'num_requests', 'html', 'css', 'javascript', 'images', 'fonts', 'transferred_bytes'
]
...
print("=== VOLLSTÄNDIGKEITSPRÜFUNG: HAR-METRIKEN ===")
...
if missing_metric_columns:
    print(
        "\nFehlende Spalten in der HAR-Metrikdatei: "
...
resource_sum = numeric_metrics[RESOURCE_COLUMNS].sum(axis=1)

resource_sum_too_large = (
    resource_sum > numeric_metrics["num_requests"]
) & found.eq(True)

size_without_requests = (
    numeric_metrics["num_requests"].eq(0)
    & numeric_metrics["transferred_bytes"].gt(0)
    & found.eq(True)
)

no_requests = (
    numeric_metrics["num_requests"].le(0)
    & found.eq(True)
)
...
print(
    "Summe der Ressourcentypen größer als total_requests: "
    f"{resource_sum_too_large.sum()}"
)
print(
    "Übertragene Datenmenge vorhanden, aber keine Requests: "
    f"{size_without_requests.sum()}"
)
print(
    f"Gefundene Crawls ohne Requests: "
    f"{no_requests.sum()}"
)
print(
...
all_correct = (
    ...
    and not resource_sum_too_large.any()
    and not size_without_requests.any()
    and not no_requests.any()
    and valid_rows.all()
)
...
if all_correct:
    print("Die HAR-Metrikdatei ist vollständig und plausibel.")
else:
    print("Die HAR-Metrikdatei enthält Auffälligkeiten.")
